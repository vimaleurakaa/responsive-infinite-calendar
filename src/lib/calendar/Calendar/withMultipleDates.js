...
var withMultipleDates = _compose(
  withDefaultProps,
  _withState("scrollDate", "setScrollDate", getInitialDate),
  _withState("displayDate", "setDisplayDate", getInitialDate),
...
export function defaultMultipleDateInterpolation(date, selected) {
  ...
  return index === -1
    ? [].concat(selected, [date])
    : [].concat(selected.slice(0, index), selected.slice(index + 1));
}
