  ...
  return {
    className:
      ...
      classNames(
        styles.range,
        ((_classNames = {}),
        (_classNames[styles.start] = isStart),
        (_classNames[styles.betweenRange] = !isStart && !isEnd),
        (_classNames[styles.end] = isEnd),
        _classNames)
      ),
...
var withRange = _compose(
  withDefaultProps,
  _withState("scrollDate", "setScrollDate", getInitialDate),
  _withState("displayKey", "setDisplayKey", getInitialDate),
  _withState("selectionStart", "setSelectionStart", null),
  withImmutableProps(function (_ref2) {
              ...
              !isTouchDevice && props.selectionStart
                ? function (e) {
                    return handleMouseOver(
                      e,
                      _extends({ selected: selected }, props)
                    );
                  }
                : null,
          },
        ...
        Years: {
          ...
          onSelect: function onSelect(date) {
            return handleYearSelect(
              date,
              _extends({ displayKey: displayKey, selected: selected }, props)
            );
          },
        },
        Header: {
  ...
  onSelect(
    getSortedSelection(
      Object.assign(
        ...
        selected,
        ((_Object$assign = {}),
        (_Object$assign[displayKey] = parse(date)),
        _Object$assign)
      )
    )
  );
